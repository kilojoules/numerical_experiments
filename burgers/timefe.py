...
if __name__=="__main__":
     ...
     dts = np.array([8e-6, 6e-6, 5e-6, 4e-6, 2e-6, 1e-6]) * 1e-5
     maxdt = geturec(x, nu=NU, evolution_time=ET, timestrategy=TS, returndt=True)
     if np.max(dts) > maxdt: raise(Exception("Bad time")) ; quit()
     trueu = geturec(x, nu=NU, evolution_time=ET, dt=dts[-1] / 100., n_save_t=1, timestrategy=TS)[:, -1]
     y = []
     ...
     a = mini(lambda x: 1e15 * (np.exp(x)*dts[-1] ** 2 -y[-1])**2, [20]).x
