...
dx = x[1] - x[0]
u_record = geturec(nu=nu, evolution_time = .5, x=x, strategy='4c')
...
nu = 0.05
u_baseline = geturec(nu=nu, evolution_time = .5, x=x, strategy='4c')
ubartest = u_baseline.mean(1)
u_baseline = (u_baseline.T - ubartest).T.copy()
#@nb.jit
def genterms(MODES):
    bk1 = np.zeros(MODES)
    bk2 = np.zeros(MODES)
    ...
    for kk in range(MODES):
        #bk1[kk] += simps(nu * psi[:, kk] *  d2(ubar, dx)) * dx
        #bk2[kk] += -1 * simps(ubar * psi[:, kk] *  d1(ubar, dx), dx=dx)
        bk1[kk] += nu * psi[:, kk].dot(d2(ubar, dx))
        bk2[kk] += -1 * ubar.dot(psi[:, kk] * d1(ubar, dx))
        for ii in range(MODES):
            #Lik1[ii, kk] += nu * simps(psi[:, kk] * d2(psi[:, ii], dx), dx=dx)
            #Lik2[ii, kk] += -1 * simps(ubar * psi[:, kk] *  d1(psi[:, ii], dx)
            #                         + psi[:, ii] * psi[:, kk] * d1(ubar, dx), dx=dx)
            Lik1[ii, kk] += nu * psi[:, kk].dot(d2(psi[:, ii], dx))
            Lik2[ii, kk] += (-1 * ubar.dot(psi[:, kk] * d1(psi[:, ii], dx))
                                     + psi[:, ii].dot(psi[:, kk] * d1(ubar, dx)))
            for jj in range(MODES):
                Nijk[ii, jj, kk] += -1 * psi[:, kk].dot(psi[:, ii] * d1(psi[:, jj], dx))
                #Nijk[ii, jj, kk] += -1 * simps(psi[:, kk] * psi[:, ii] * d1(psi[:, jj], dx), dx=dx)
...
####################################################
plt.plot(x, ubartest + u_baseline[:, -1], c='k')
plt.plot(x, ubar + u_red_record[:, -1], c='r')
plt.plot(x, ubartest + u_baseline[:, 0], c='k', ls='-.')
plt.plot(x, ubar + u_red_record[:, 0], c='r', ls='--')
plt.title('%i Modes, nu=%f, nx=%i'%(MODES, nu, nx))
plt.savefig('u_testpred_%im%ix%f.pdf'%(MODES, nx, nu))
plt.clf()
